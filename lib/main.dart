...
class _HomeState extends State<Home> {
  ...
  void initSpeechRecognizer() {
    ...
    _speechRecognition
        .activate()
        .then((result) => setState(() => _estaDisponible = result));
  }
  ...
  iniciarSpeech() {
    // -------------- Aqui iniciamos el Speech al presionar el icono de Voz
    ...
    Timer(Duration(seconds: 4), () {
      // ------------ Ponemos un temporizador predeterminado
      _enviarMensaje();
    });
  }

  void _enviarMensaje() async {
    // -------------------- Enviamos el msj cuando termine el temporizador y lo guardamos en la variable respuesta
    setState(() {
      cuerpo = Text("");
    });
    print("entro y tu msj es " + contenido);
    AuthGoogle authGoogle =
        await AuthGoogle(fileJson: "assets/cred2.json").build();
    ...
    if (respuesta != "") {
      //--------------------------------- Si la respuesta no es un HTTP, entonces dimelo
  ...
  void escuchar(String cad) async {
    //------------------------- Texto a Voz
